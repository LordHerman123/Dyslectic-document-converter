...
* **Publishing a release:** push a tag such as `v1.6.0` (or create a release on GitHub with that tag). The
  ...
  * `DyslexiaConverter-1.6.0-setup.exe` — an installer (per user, no administrator rights needed), or
  * `DyslexiaConverter-1.6.0-windows.zip` — unzip anywhere and double-click `DyslexiaConverter.exe`.
...
python windows\build.py                    # -> dist\DyslexiaConverter\ and dist\DyslexiaConverter-1.6.0-windows.zip
iscc windows\installer.iss                 # optional -> dist\DyslexiaConverter-1.6.0-setup.exe
```
