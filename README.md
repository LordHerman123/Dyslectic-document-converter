...
| OCR | Tesseract (English, Dutch, and other installed languages). Rebuilds lines, paragraphs, headings and reading order. Pictures and ruled tables on scanned pages are kept as images. |
| OCR correction | Uses a local dictionary (pyspellchecker) plus the typical OCR mix-ups (`rn`→`m`, `l`→`i`, `0`→`o`, …). Only high-confidence fixes are applied automatically. Modes: Automatic, Review (Accept/Reject in the app) and Off. Names, abbreviations, identifiers, British/American spellings and your own dictionary words are left alone. |
